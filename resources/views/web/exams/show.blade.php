...
@section('main')
    ...
    <div class="hero-area section">
        ...
        <!-- Backgound Image -->
        <div class="bg-image bg-parallax overlay" style="background-image:url({{asset("web/assets/img/blog-post-background.jpg")}})"></div>
        <!-- /Backgound Image -->
                        ...
                        {{-- dispaly start button only for auth user ,who is student & dont solve it before --}}
                            ...
                            @if($canViewStartBtn )
                                <form action="{{url("/exams/start/{$exam->id}")}}" method="POST">
                                    @csrf
                                    <button type="submit" class="main-button icon-button pull-left">{{__('web.startExam')}}</button>
                                </form>
                            @endif
